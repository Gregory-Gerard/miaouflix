...
export const url = (path: string, searchParams: Record<string, string> = {}) => {
  invariant(process.env.TMDB_API_KEY);
  invariant(process.env.TMDB_API_URL);

  const generatedUrl = new URL(`${process.env.TMDB_API_URL}/${path.trim().replace(/^\/+/, '')}`);
  ...
  Object.entries(searchParams).map(([key, value]) => generatedUrl.searchParams.append(key, value));
